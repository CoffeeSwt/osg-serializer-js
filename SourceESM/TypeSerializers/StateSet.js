...
let objectWrapper = new ObjectWrapper('osg::StateSet', [
    'osg::Object',
    'osg::StateSet'
], StateSet);
...
function readValue(inputStream) {
    if (inputStream.isBinary())
        ...
        let value = 0;
        if (enumValue.indexOf('OFF') >= 0)
            value = StateAttributeValueEnum.OFF;
        if (enumValue.indexOf('ON') >= 0)
            value = StateAttributeValueEnum.ON;
        if (enumValue.indexOf('OVERRIDE') >= 0)
            value = value | StateAttributeValueEnum.OVERRIDE;
        if (enumValue.indexOf('PROTECTED') >= 0)
            value = value | StateAttributeValueEnum.PROTECTED;
        if (enumValue.indexOf('INHERIT') >= 0)
            value = value | StateAttributeValueEnum.INHERIT;
        return value;
    }
}
